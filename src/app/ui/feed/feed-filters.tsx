...
export default function FeedCategoryTabs({ categories, selected, counts, totalCount, onToggle, onSelectAll }: FeedCategoryTabsProps) {
  return (
    <nav className="tabs feedCategoryTabs" aria-label="카테고리 필터">
      ...
      {categories.map((c) => (
        ...
        >
          {c.label} <span>{counts.find((x) => x.category === c.id)?.count || 0}</span>
        </button>
      ))}
    </nav>
  );
