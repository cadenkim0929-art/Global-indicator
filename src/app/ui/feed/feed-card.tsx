  ...
  return (
    <article className="feedCard" style={{ ['--accent' as string]: categoryColor, ['--i' as string]: Math.min(index, 12) }}>
      ...
      <h3 className="feedCardTitle">
        {cleanedTitle}
        {secondaryTitle && <span className="feedCardTitleEn">{secondaryTitle}</span>}
      </h3>
      ...
      <div className="feedCardFooter">
        <div className="feedCardFooterLeft">
          ...
          <span className="feedCardScore" title={t(lang, '영향도 점수', 'Impact score')}>Impact {Math.round(article.score || 0)}</span>
        </div>
        <div className="feedCardFooterRight">
          <button type="button" className="feedBookmarkBtn" disabled aria-label={t(lang, '관심 기사 저장(준비 중)', 'Save article (coming soon)')} title={t(lang, '관심 기사 저장 기능은 준비 중입니다', 'Save article is coming soon')}>
          ...
          <a href={article.link} target="_blank" rel="noreferrer" className="feedCardLink">{t(lang, '원문 ↗', 'Original ↗')}</a>
        </div>
      </div>
    </article>
  );
